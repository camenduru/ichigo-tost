...
RUN pip install -q opencv-python imageio imageio-ffmpeg ffmpeg-python av runpod \
    ...
    pip install xformers==0.0.27 whisperspeech matplotlib openai-whisper==20231117 git+https://github.com/homebrewltd/WhisperSpeech.git vector_quantize_pytorch bitsandbytes transformers accelerate webdataset && \
    ...
    curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.7/install.sh | bash && export NVM_DIR="$HOME/.nvm" && [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh" && nvm install 18.20.3 && \
    cd /content/ichigo-demo && npm install && npm run build

WORKDIR /content
CMD cd /content/tabbyAPI && python main.py && cd /content/fish-speech/tools && uvicorn 'api:app' --workers 3 --host 0.0.0.0 --port 22311 && python whisperAPI.py && cd /content/ichigo-demo && npm start