...
def inference(req: ServeTTSRequest):
    ...
    if idstr is not None:
        ...
        prompt_texts = [
            read_ref_text(str("references/ref.txt"))
            # for ref_audio in ref_audios
        ]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/content/fish-speech/checkpoints/fish-speech-1.4",
    )
    parser.add_argument(
        "--decoder-checkpoint-path",
        type=str,
        default="/content/fish-speech/checkpoints/fish-speech-1.4/firefly-gan-vq-fsq-8x1024-21hz-generator.pth",
    )
...
class MsgPackRequest(HttpRequest):
    async def data(self) -> Annotated[Any, ContentType("application/json")]:
        if self.content_type == "application/json":
            body = await self.body
            return json.loads(body) #ormsgpack.unpackb(await self.body)
